...
exports.testService = async () => {
  ...
  return { auth: 'test route working!' }
}
...
exports.getUserService = async (id) => {
  logger.debug('0️⃣  calling user endpoint with user id: %o', id)
...
exports.forgotPassService = async (id) => {
  ...
  const updateUser = await user.updateOne({ 'resetToken': resetPassToken })
...
exports.signOutService = async (userInput, id) => {
  return
}
